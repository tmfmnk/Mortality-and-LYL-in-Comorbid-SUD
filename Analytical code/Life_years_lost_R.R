...
#Import mortality tables for year 2006 (the middle year between 1999 and 2017)

mortality_table_males <- fread(file = "/path",
                               select = c("age", 
                                          "mortality_rates",
                                          "survival"),
                               dec = ",")

mortality_table_females <- fread(file = "/path",
                                 select = c("age", 
                                            "mortality_rates",
                                            "survival"),
                                 dec = ",")
...
lyl_males_30_SUD <- map2_dfr(.x = data_lyl_males_SUD[ind_males_30_SUD],
                             .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_males_30_SUD],
                             ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                            ...
                                            age = age,
                                            weights = .x$age_first_hosp_comorbid_condition,
                                            surv = survival) %>%
                                as.data.frame() %>%
...
lyl_males_30_no_SUD <- map2_dfr(.x = data_lyl_males_no_SUD[ind_males_30_no_SUD],
                                .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_males_30_no_SUD],
                                ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                          t0 = age_first_hosp_comorbid_condition,
                                                          t = age_death_or_censoring,
                                                          status = mortality,
...
lyl_females_30_SUD <- map2_dfr(.x = data_lyl_females_SUD[ind_females_30_SUD],
                               .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_females_30_SUD],
                               ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                         t0 = age_first_hosp_comorbid_condition,
                                                         t = age_death_or_censoring,
                                                         status = mortality,
...
lyl_females_30_no_SUD <- map2_dfr(.x = data_lyl_females_no_SUD[ind_females_30_no_SUD],
                                  .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_females_30_no_SUD],
                                  ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                            t0 = age_first_hosp_comorbid_condition,
                                                            t = age_death_or_censoring,
                                                            status = mortality,
...
lyl_males_45_SUD <- map2_dfr(.x = data_lyl_males_SUD[ind_males_45_SUD],
                             .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_males_45_SUD],
                             ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                       t0 = age_first_hosp_comorbid_condition,
                                                       t = age_death_or_censoring,
                                                       status = mortality,
...
lyl_males_45_no_SUD <- map2_dfr(.x = data_lyl_males_no_SUD[ind_males_45_no_SUD],
                                .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_males_45_no_SUD],
                                ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                          t0 = age_first_hosp_comorbid_condition,
                                                          t = age_death_or_censoring,
                                                          status = mortality,
...
lyl_females_45_SUD <- map2_dfr(.x = data_lyl_females_SUD[ind_females_45_SUD],
                               .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_females_45_SUD],
                               ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                         t0 = age_first_hosp_comorbid_condition,
                                                         t = age_death_or_censoring,
                                                         status = mortality,
...
lyl_females_45_no_SUD <- map2_dfr(.x = data_lyl_females_no_SUD[ind_females_45_no_SUD],
                                  .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_females_45_no_SUD],
                                  ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                            t0 = age_first_hosp_comorbid_condition,
                                                            t = age_death_or_censoring,
                                                            status = mortality,
...
lyl_males_60_SUD <- map2_dfr(.x = data_lyl_males_SUD[ind_males_60_SUD],
                             .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_males_60_SUD],
                             ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                       t0 = age_first_hosp_comorbid_condition,
                                                       t = age_death_or_censoring,
                                                       status = mortality,
...
lyl_males_60_no_SUD <- map2_dfr(.x = data_lyl_males_no_SUD[ind_males_60_no_SUD],
                                .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_males_60_no_SUD],
                                ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                          t0 = age_first_hosp_comorbid_condition,
                                                          t = age_death_or_censoring,
                                                          status = mortality,
...
lyl_females_60_SUD <- map2_dfr(.x = data_lyl_females_SUD[ind_females_60_SUD],
                               .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_females_60_SUD],
                               ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                         t0 = age_first_hosp_comorbid_condition,
                                                         t = age_death_or_censoring,
                                                         status = mortality,
...
lyl_females_60_no_SUD <- map2_dfr(.x = data_lyl_females_no_SUD[ind_females_no_60],
                                  .y = map(.x = data_models, ~ sub("_first_comorbid_hosp_end_date_year", "", names(select(.x, 3))))[ind_females_no_60],
                                  ~ lyl_diff_ref(lyl_ci(lyl(data = .x,
                                                            t0 = age_first_hosp_comorbid_condition,
                                                            t = age_death_or_censoring,
                                                            status = mortality,
